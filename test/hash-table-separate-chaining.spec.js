...
import { HashTableSeparateChaining } from '../src/javascript/data-structures/hash-table-separate-chaining'
import MyObj from './my-obj'
import { defaultToString } from '../src/javascript/utils'





describe('使用链表解决过的hash-table的测试文件', () => {
  const A = 'Jonathan';
  const B = 'Jamie';
  const C = 'Sue';
  ...
  it('puts values with number key without collisions', () => {
    ...
    for (let i = min; i <= max; i++) {
      ...
      expect(valuePair.ele.key).to.equal(i);
      expect(valuePair.ele.value).to.equal(i);
    }
  });
  ...
  it('puts values with string key without collisions', () => {
    ...
    expect(valuePair.ele.key).to.equal('1');
    expect(valuePair.ele.value).to.equal(1);
    linkedList = table[23]; // eslint-disable-line prefer-destructuring
    expect(linkedList.size()).to.equal(1);
    valuePair = linkedList.getHead();
    expect(valuePair.ele.key).to.equal('10');
    expect(valuePair.ele.value).to.equal(10);
    linkedList = table[34]; // eslint-disable-line prefer-destructuring
    expect(linkedList.size()).to.equal(1);
    valuePair = linkedList.getHead();
    expect(valuePair.ele.key).to.equal('100');
    expect(valuePair.ele.value).to.equal(100);
    linkedList = table[8]; // eslint-disable-line prefer-destructuring
    expect(linkedList.size()).to.equal(1);
    valuePair = linkedList.getHead();
    expect(valuePair.ele.key).to.equal('1000');
    expect(valuePair.ele.value).to.equal(1000);
  });
  it('puts values with object key without collisions', () => {
    ...
    expect(linkedList.size()).to.equal(1);
    let valuePair = linkedList.getHead();
    expect(valuePair.ele.key).to.equal(myObjList[0]);
    expect(valuePair.ele.value).to.equal(myObjList[0]);
    linkedList = table[3]; // eslint-disable-line prefer-destructuring
    expect(linkedList.size()).to.equal(1);
    valuePair = linkedList.getHead();
    expect(valuePair.ele.key).to.equal(myObjList[1]);
    expect(valuePair.ele.value).to.equal(myObjList[1]);
    linkedList = table[5]; // eslint-disable-line prefer-destructuring
    expect(linkedList.size()).to.equal(1);
    valuePair = linkedList.getHead();
    expect(valuePair.ele.key).to.equal(myObjList[2]);
    expect(valuePair.ele.value).to.equal(myObjList[2]);
    linkedList = table[7]; // eslint-disable-line prefer-destructuring
    expect(linkedList.size()).to.equal(1);
    valuePair = linkedList.getHead();
    expect(valuePair.ele.key).to.equal(myObjList[3]);
    expect(valuePair.ele.value).to.equal(myObjList[3]);
    linkedList = table[9]; // eslint-disable-line prefer-destructuring
    expect(linkedList.size()).to.equal(1);
    valuePair = linkedList.getHead();
    expect(valuePair.ele.key).to.equal(myObjList[4]);
    expect(valuePair.ele.value).to.equal(myObjList[4]);
  });
  ...
  it('puts values with collisions', () => {
    ...
    for (let i = min; i <= max; i++) {
      ...
      expect(valuePair.ele.key).to.equal(i);
      expect(valuePair.ele.value).to.equal(i + 10);
      valuePair = valuePair.next;
      expect(valuePair.ele.key).to.equal(i);
      expect(valuePair.ele.value).to.equal(i + 100);
    }
  });
