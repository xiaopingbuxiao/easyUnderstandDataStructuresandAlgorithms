...
import { DoublyLinkedList } from '../src/javascript/data-structures/doubly-linked-list'
...
describe('双向链表的测试', () => {
  ...
  function verifyNode(current, i) {
    ...
    if (i > min) {
      ...
      if (current.prev) {
        expect(current.prev.ele).to.equal(i - 1);
      }
    } else {
  ...
  it('removes element from middle of list', () => {
    ...
    if (current) {
      ...
      expect(current.prev).to.not.be.an('undefined');
      if (current.prev) {
        expect(current.prev.ele).to.equal(1);
      }
    }
  });
