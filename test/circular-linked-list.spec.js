...
import { CircularLinkedList } from '../src/javascript/data-structures/circular-linked-list'
import MyObj from './my-obj'


describe('单向链表的循环链表处理', () => {
  let list;
  let min;
  let max;
    ...
    for (let i = min; i <= max && current; i++) {
      ...
      if (current) {
        ...
        if (i < max) {
          ...
          if (current.next) {
            expect(current.next.ele).to.equal(min);
          }
        }
  ...
  it('removes element from middle of list', () => {
    ...
    if (current) {
      expect(current.ele).to.not.be.an('undefined');
      expect(current.ele).to.equal(min);
      expect(current.next).to.not.be.an('undefined');
      console.log(current.ele)
      if (current.next) {
        expect(current.next.ele).to.equal(max);
        current = current.next;
      }
    }
    ...
    if (current) {
      expect(current.ele).to.not.be.an('undefined');
      expect(current.ele).to.equal(max);
      expect(current.next).to.not.be.an('undefined');
      expect(current.next).to.equal(list.getHead());
      if (current.next) {
        expect(current.next.ele).to.equal(min);
